...
Settings preferences (language, theme, kg or lb, entry mode, Load the bar) belong to the account and follow the person to another phone; theme defaults to following the device. Only the bar weight picked on a Train card stays on that phone, remembered per exercise.

Every preference is read through prefs(), which supplies the default.
