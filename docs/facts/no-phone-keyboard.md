...
Train never opens the phone's keyboard: weight and reps are set with Buttons (default) or a Ruler, and tapping a number opens gymmy's own keypad. 'Load the bar' (on by default) shows barbell weight as plates, and the bar weight is remembered per exercise on that phone only.
