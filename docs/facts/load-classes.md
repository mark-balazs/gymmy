...
Every exercise declares a load class: barbell, dumbbell pair, single dumbbell, machine (Smith machine included), bodyweight or partial load (landmine, sled). Machine, added-bodyweight and partial loads are not real masses: they are charted against themselves and never reach DOTS, though the strength index still takes a pattern's best from them.
