...
Progress shows two strength numbers: gymmy's strength index ('am I getting stronger?') and DOTS ('how do I compare?'). Both come from eight weeks of estimates (the code counts nine until GYM-46 lands) and read higher than a meet total, which the screen says; the index shows in the tens with one decimal, DOTS as a whole number in the hundreds.
