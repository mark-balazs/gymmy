...
4. Scrolled down a long list, pulling down scrolls the list back up. Only at
   the top does it move the sheet.
...
- **It follows the finger**: a hundred pixels of drag moves it a hundred
  pixels, and nothing moves in the first few pixels, so a wobbly tap is a tap.
...
Safari (WebKit is not in the suite), and a fling that coasts into the top of a
list not turning into a dismissal.
